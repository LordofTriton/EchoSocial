...
export default async function BanCommunityMember (request, response) {
    ...
    try {
        ...
        response.once("finish", async () => {
            await axios.delete(AppConfig.getHost(request) + `/api/community-members/delete-community-member?accountID=${params.accountID}&userID=${params.userID}&communityID=${params.communityID}`)
            await axios.post(AppConfig.getHost(request) + "/api/blacklists/create-blacklist", {
                accountID: params.accountID,
